...
// Check if a node is deleted (used during search)
__device__ inline bool isDeleted(const uint32_t* d_bitmap, uint32_t node_id) {
    uint32_t word_idx = node_id / 32;
    uint32_t bit_idx = node_id % 32;
    return (d_bitmap[word_idx] & (1U << bit_idx)) != 0;
}

// Batch check if nodes are deleted (returns boolean array)
__global__ void checkDeletedKernel(const uint32_t* d_bitmap, uint32_t* d_ids,
                                   bool* d_is_deleted, uint32_t batch_size) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;

    if (idx < batch_size) {
        uint32_t node_id = d_ids[idx];
        d_is_deleted[idx] = isDeleted(d_bitmap, node_id);
    }
}

// Count total deleted nodes (parallel reduction)
__global__ void countDeletedKernel(const uint32_t* d_bitmap, uint32_t* d_count,
                                   uint32_t bitmap_words) {
    __shared__ uint32_t shared_count[THREADS_PER_BLOCK];
...
void initDeleteBuffer(DeleteBuffer* buffer, uint32_t total_capacity) {
    ...
    uint32_t bitmap_words = (total_capacity + 31) / 32;  // Round up to nearest word
    ...
    checkCudaError(err, "Failed to allocate device delete bitmap");

    err = cudaMemset(buffer->d_bitmap, 0, buffer->bitmap_size_bytes);
    checkCudaError(err, "Failed to initialize device delete bitmap");
...
void deleteBatch(DeleteBuffer* del_buf, uint32_t* h_ids, uint32_t batch_size) {
    ...
    checkCudaError(err, "Failed to allocate device memory for delete IDs");

    // Copy IDs to device
    err = cudaMemcpy(d_ids, h_ids, batch_size * sizeof(uint32_t),
                     cudaMemcpyHostToDevice);
    checkCudaError(err, "Failed to copy delete IDs to device");

    // Launch kernel to mark deleted
    uint32_t num_blocks = (batch_size + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
    markDeletedKernel<<<num_blocks, THREADS_PER_BLOCK>>>(
        del_buf->d_bitmap, d_ids, batch_size);

    err = cudaDeviceSynchronize();
    checkCudaError(err, "Delete kernel failed");

    // Update host bitmap (for debugging/verification)
    err = cudaMemcpy(del_buf->h_bitmap, del_buf->d_bitmap,
                     del_buf->bitmap_size_bytes, cudaMemcpyDeviceToHost);
    checkCudaError(err, "Failed to copy delete bitmap to host");
...
void clearDeleteBuffer(DeleteBuffer* buffer) {
    ...
    checkCudaError(err, "Failed to clear delete bitmap");
...
uint32_t countDeleted(DeleteBuffer* buffer) {
    ...
    checkCudaError(err, "Failed to allocate device counter");
    ...
    uint32_t num_blocks = (bitmap_words + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;

    countDeletedKernel<<<num_blocks, THREADS_PER_BLOCK>>>(
        buffer->d_bitmap, d_count, bitmap_words);

    err = cudaDeviceSynchronize();
    checkCudaError(err, "Count deleted kernel failed");

    // Copy result back
    uint32_t count;
    err = cudaMemcpy(&count, d_count, sizeof(uint32_t), cudaMemcpyDeviceToHost);
    checkCudaError(err, "Failed to copy count to host");
